...
export const NotificationPopup: React.FC<NotificationPopupProps> = ({ alert, onClose }) => {
  ...
  useEffect(() => {
    if (alert) {
      ...
      const timer = setTimeout(() => {
        setShow(false);
        setTimeout(() => { if (onCloseRef.current) onCloseRef.current(); }, 500);
      }, 4000);
  ...
  const displayName = symbolInfo ? symbolInfo.name : alert.symbol.replace(/_otc/gi, ' (OTC)');
  ...
  return (
    <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] transition-all duration-500 ease-in-out ${show ? 'translate-y-0 opacity-100' : '-translate-y-12 opacity-0'}`}>
      <div className={`relative overflow-hidden bg-[#1e222d] border ${isWarning ? 'border-yellow-500/50' : 'border-[#2a2e39]'} shadow-2xl rounded-2xl w-[90vw] max-w-sm backdrop-blur-xl`}>
        ...
        <div className="flex items-center p-3 gap-3">
          <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${iconBg}`}>
            {isWarning ? <AlertTriangle className={`w-5 h-5 ${iconColor}`} /> : isCall ? <TrendingUp className={`w-5 h-5 ${iconColor}`} /> : <TrendingDown className={`w-5 h-5 ${iconColor}`} />}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between">
              <span className={`text-base font-black tracking-wider ${typeTextColor}`}>{alert.title || alert.type}</span>
              {alert.entryTime && (
